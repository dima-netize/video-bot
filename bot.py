...
def _instagram_extract_video(html: str) -> str | None:
    """Витягає URL відео з HTML сторінки Instagram-дзеркала."""
    for pat in [
        r'<video[^>]+src="([^"]+)"',
...
def instagram_fallback(url: str, progress_cb=None, cancel_event: Event | None = None):
    """Декілька Instagram fallback дзеркал по черзі."""

    # Витягуємо чистий шлях без параметрів
    clean_url = re.sub(r"\?.*$", "", url.rstrip("/"))

    # Список дзеркал що не потребують cookies
    mirrors = [
        # ddinstagram — виправлена заміна (тільки один re.sub без подвоєння)
        re.sub(r"(?:www\.)?instagram\.com", "ddinstagram.com", clean_url),
        # instagramez
        re.sub(r"(?:www\.)?instagram\.com", "instagramez.com", clean_url),
        # imginn API
        None,  # спеціальний обробник нижче
    ]
    ...
    # 1. Спробуємо дзеркала
    for mirror_url in [m for m in mirrors if m]:
        try:
            if progress_cb:
                domain = re.search(r"://([^/]+)", mirror_url)
                progress_cb(f"🔁 Instagram через {domain.group(1) if domain else 'дзеркало'}...")
            r = requests.get(mirror_url, headers=headers, timeout=REQUEST_TIMEOUT, allow_redirects=True)
            r.raise_for_status()
            video_url = _instagram_extract_video(r.text)
            if video_url:
                return stream_download(
                    video_url,
            ...
            log.debug("Instagram mirror %s failed: %s", mirror_url, e)

    # 2. Спробуємо igdownloader API
    try:
        if progress_cb:
            progress_cb("🔁 Instagram через igdownloader...")
        post_id = re.search(r"/(?:reel|p)/([A-Za-z0-9_-]+)", url)
        if post_id:
            api_r = requests.get(
                f"https://igdownloader.app/api/instagram?url={url}",
                headers=headers, timeout=REQUEST_TIMEOUT,
            )
            if api_r.ok:
                data = api_r.json()
                video_url = (data.get("video") or [None])[0] if isinstance(data.get("video"), list) else data.get("video")
                if video_url:
                    return stream_download(
                        video_url,
                        safe_filename("instagram", url),
        ...
        log.debug("igdownloader failed: %s", e)

    # 3. Спробуємо через yt-dlp з Instagram-специфічними налаштуваннями
    try:
        if progress_cb:
        ...
        log.debug("Instagram yt-dlp failed: %s", e)

    return None, "❌ Не вдалося завантажити Instagram відео. Можливо, відео приватне або потрібні cookies.txt."
...
def download_media(
    ...
    # ── TikTok: спочатку fallback без watermark ──────────────────
    if platform == "tiktok" and not audio:
        if progress_cb: progress_cb("🔁 TikTok (no-watermark)...")
        p, r = tiktok_fallback_tikwm(url, progress_cb, cancel_event)
        if p: return p, r
        if progress_cb: progress_cb("🔁 TikTok fallback #2...")
        p, r = tiktok_fallback_snaptik(url, progress_cb, cancel_event)
        if p: return p, r
        if progress_cb: progress_cb("🔁 TikTok через yt-dlp...")

    # ── Instagram: спочатку fallback (без cookies) ────────────────
    if platform == "instagram" and not audio:
        p, r = instagram_fallback(url, progress_cb, cancel_event)
        if p: return p, r
        # якщо всі fallback провалились — пробуємо yt-dlp з cookies
        if progress_cb: progress_cb("🔁 Instagram через yt-dlp...")

    # ── yt-dlp з авто-ретраєм ────────────────────────────────────
    last_error = ""
    for attempt in range(1, MAX_RETRIES + 1):
        if cancel_event and cancel_event.is_set():
            return None, "Завантаження скасовано."
        if attempt > 1:
            if progress_cb: progress_cb(f"🔁 Спроба {attempt}/{MAX_RETRIES}...")
            time.sleep(2 ** (attempt - 1))
        p, r = download_via_ytdlp(url, platform, audio, quality, progress_cb, cancel_event)
        if p: return p, r
        last_error = r
        if not is_transient_error(r): break
